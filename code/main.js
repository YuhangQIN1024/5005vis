...
class Main {
    constructor() {
        ...
        this._skeys = ['appearances', 'blocks', 'steals'];
        this._bkeys = ['total_time', 'points', 'rebounds', 'assists'];
        this._pkeys = ['FG%', '3P%'];
        this._begin_end = [-1, Infinity];
        this._thisteam = "Both Teams";
    ...
    team_dom_uptime() {
        ...
        let ans = this._teams.then(function (teams) {
            ...
            <input type="radio" name="team" class="chooseteam" id="chooseteam-both" value="Both teams" ${if_choose_1_2}/>
            <label for="chooseteam-both" id="both">Both teams</label>
            ...
            if (chooseplayer !== null) {
                ...
                Promise.all([player_stats0, all_player_stats]).then(function ([player_stats0, all_player_stats]) {
                    ...
                    function xz(keys, di, big, f) {
                        const sc = [['product', player_stats["name"], 'League Average', 'League Maximum']];
                        for (const i in keys) {
                            const key = keys[i];
                            sc.push([key, player_stats[key], all_player_stats['avg_' + key], all_player_stats['max_' + key]]);
                        }
                ...
                Promise.all([all_player_stats]).then(function ([all_player_stats]) {
                    function wxz(keys, di, big, f) {
                        const sc = [['product', 'League Average', 'League Maximum']];
                        for (const i in keys) {
                            const key = keys[i];
                            sc.push([key, all_player_stats['avg_' + key], all_player_stats['max_' + key]]);
                        }
                        Promise.all([_this._theme]).then(function ([themeJSON]) {
                            echarts.registerTheme('vintage', themeJSON);
                            var myChart = echarts.init(document.getElementById(di), 'vintage');
